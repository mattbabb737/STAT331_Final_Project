...
response_variance <- var(augmented_data$mean_child_mortality, na.rm = TRUE)
...
noise <- function(x, mean = 0, sd) {
  return(tibble(x + rnorm(length(x),
            mean,
            sd)
            )
         )
...
simulations <- map(1:1000, ~noise(predictions, sd = st_error))
