...
```{r}
# Plot 2
yearly_mean_child_mortality <- final_data %>%
  group_by(year) %>%
  summarise(
    mean_child_mortality = mean(child_mortality, na.rm = TRUE),
    mean_emissions = mean(emissions, na.rm = TRUE)
  ...
  geom_line(aes(y = mean_child_mortality)) +
  labs(x = "Year", y = "Mean Child Mortality", title = "Global Yearly Trends") +
  theme_bw()

# Plot_3
yearly_mean_emissions <- final_data %>%
  group_by(year) %>%
  summarise(
    mean_child_mortality = mean(child_mortality, na.rm = TRUE),
    mean_emissions = mean(emissions, na.rm = TRUE)
  ...
  ggplot(mapping = aes(x = year)) +
  geom_line(aes(y = mean_emissions)) +
  labs(x = "Year", y = "Mean Emissions", title = "Global Yearly Trends") +
  theme_bw()
...
ggplot(yearly_means, mapping = aes(x = year)) +
  geom_line(aes(y = mean_child_mortality, color = "Child Mortality")) +
  geom_line(aes(y = mean_emissions, color = "Emissions")) +
  labs(x = "Year", 
       y = "Mean Value", 
       title = "Global Yearly Trends") +
  scale_color_manual(values = c("Child Mortality" = "red", "Emissions" = "blue")) +
  theme_bw()

yearly_mean_child_mortality
yearly_mean_emissions
yearly_means
```
