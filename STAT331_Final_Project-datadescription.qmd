...
```
**1. Linearity:**  
By looking at the above scatterplot from section III.a, a clear linear pattern is evident. The Pearson's correlation coefficient (r) is `r r_yearly`. Given these observations, the first criteria failed to be violated.
...
**3. Normality of residuals:**  
The histogram below displays the distribution of residuals from the model. The distribution is approximately centered around zero, suggesting that the model does not have systematic bias. The overall shape of the distribution approximates a bell curve with a modest left skew. These features do not violate the assumption of normality, especially considering the large sample size.
